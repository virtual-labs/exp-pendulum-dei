...
const checkBox = () => {
    if (lockButton.checked) {
        ...
        timePeriod = 2 * Math.PI * Math.sqrt(pendulum.length / 100 / pendulum.gravity);
        timeperiodSet = [];
...
const updatePendulum_inputs_comp = (input, key, displayId, isAngle = false) => {
    comp_pendulum[key] = isAngle ? parseFloat(input.value) * (Math.PI / 180) : parseFloat(input.value);
    if (displayId) getElement(displayId).innerText = input.value;
    drawCompoundPendulum();
};

comp_length_input.addEventListener("input", () => updatePendulum_inputs_comp(comp_length_input, "length", "length_comp_val"));
comp_width_input.addEventListener("input", () => updatePendulum_inputs_comp(comp_width_input, "width", "width_comp_val"));
comp_radius_input.addEventListener("input", () => updatePendulum_inputs_comp(comp_radius_input, "radius", "radius_comp_val"));
comp_angle_input.addEventListener("input", () => updatePendulum_inputs_comp(comp_angle_input, "angle", "angle_comp_val", true));
comp_oscillations_input.addEventListener("input", () => updatePendulum_inputs_comp(comp_oscillations_input, "maxOscillations_comp"));
comp_mass_input.addEventListener("input", () => updatePendulum_inputs_comp(comp_mass_input, "mass", "mass_comp_val"));
...
const change = (radio) => {
    ...
    // Toggle visibility
    ["leftPane", "cnv", "rightPane"].forEach(id => getElement(id).hidden = isCompound);
    ["leftPane2", "rightPane2", "cnv2", "lock_comp"].forEach(id => getElement(id).hidden = !isCompound);
    // Draw pendulum
...
const checkBox_comp = () => {
    ...
    if (lockButton_comp.checked) {
        ...
        setText("comp_inertia_theoritcal", `Theoretical MOI: ${inertia.toFixed(2)} kg-m²`);
        ...
        setText("osciallations_comp_text", `No. of Oscillations: ${maxOscillations_comp}`);
        setText("pendulumType_text", `Selected Type: ${comp_pendulum.type.toUpperCase()}`);
...
function drawRectangularPlate() {
    ...
    ctx.fillRect(-comp_pendulum.width / 2, -pivotOffset, comp_pendulum.width, comp_pendulum.length);
...
function updatePendulum_comp(time) {
    comp_pendulum.angle = initialAngle_comp * Math.cos(((2 * Math.PI) / timePeriod_comp) * time)
    console.log("Comp pendulum angle", comp_pendulum.angle)
    console.log("Time period comp", timePeriod_comp);

    if (time >= timeperiodSet_comp[i_comp]) {
        oscillationCounting_comp++;
        setText("oscillationCount_comp", `Oscillation count : ${oscillationCounting_comp}`);
        i_comp++;
    }
...
function showFinalResults_comp() {
    ...
    const gravity = 9.81;
    const commonFactor = (timePeriod * timePeriod * mass * gravity) / (4 * Math.PI * Math.PI);

    let experimentalInertia = pendulumType === "disk" ?
